...
\documentclass[g5paper,11pt]{kth-bcs}
...
\usepackage[top=1.2in, bottom=0.9in, left=0.6in, right=0.6in]{geometry}
...
\businesslogo{\includegraphics[width=4cm]{Inteno}}
...
One purpose of this degree project is to achieve a more user friendly operation by adding automatic service discovery on the local network to the OpenWrt system, this will reduce the amount of support calls.
A system which identifies the ports and services running on the network devices, works alongside the preset library for port forwarding, will be integrated into the Lua Configuration Interface of OpenWrt.
...
It is assumed that operation of the device is user friendlier, where the end-user receive helpful hints in the configuration process.
...
This works by translating the gateways internal IP address to a domain name and keeping it up to date when the IP is modified.
...
Improving the graphical interface in electronic devices and its software, can cause irritations with the product and leave the user with a sense of hopelessness and self-blame.
It is important to communicate a clear usage to the user.\cite[p.~34]{norman2002design}

\begin{figure}[h!]
   \centering
   %                       left bot  right top
   \includegraphics[trim = 0mm 140mm 90mm 0mm, clip, width=15cm]{before}
   \caption{Flow chart for user interaction when selecting port forwarding on conventional firewall page}
   \label{fig:before}
\end{figure}
...
\begin{figure}[h!]
   \centering
   %                       left bot  right top
   \includegraphics[trim = 0mm 170mm 80mm 0mm, clip, width=15cm]{after}
