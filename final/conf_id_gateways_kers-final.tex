...
The main motivation behind the qualitative analysis is that the workflow is independent of producing prototypes and mockups for a testing group.
This allowed for a more sequential approach in delivering the module and provided time to familiarize with the OpenWrt ecosystem before implementing the user interface.
On request of Inteno, the basic usability scenario chosen for evaluation was configuring port forwarding of the XBox 360.

Disadvantages of not using actual usability tests with people, includes developers missing out on valuable feedback from the people they are designing it for.
