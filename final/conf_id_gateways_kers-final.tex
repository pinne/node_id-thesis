...
These parts are connected by the LuCI dispatcher\footnote{Not shown in diagram} and the model.
Communication between the different parts of the port forwarding process is outlined in figure~\ref{fig:wizard-seq_dia}.
...
A scan is then performed in the background by Nmap, which %TODO
...
The final redirection rules in the firewall configuration file are thus constructed from the preset library, the network service scan and the users choice.
...
DNS works by translating hard to remember IP addresses to memorable domain names.
By using the lightweight DNS forwarder Dnsmasq we can make the web interface easier to access.
...
The script gets its current IP address from the UCI command-line interface, it then proceeds to iterate through the lines of the hosts file, updating the IP address associated with the $login.lan$ hostname.
Since any DNS request passes through the router, Dnsmasq will now translate the human-memorable name to its own IP address.
...
xhr.get('<%=luci.dispatcher.build_url("admin","wizard")%>/get_list/'+argument, null,
    function(reply) {
      ...
      \caption{
         \small{
Example of calling the LuCI dispatcher from client-side JavaScript with an argument, using the XMLHttpRequest helper class $xhr.js$ of LuCI.
         }
      }
